...
export default function MshtasticBoilerplate() {
    ...
    try {
      ...
      meshDevice.events.onMessagePacket.subscribe((packet: any) => {
        ...
        if (text.startsWith("s=")) {
          ...
          // Balance request — queue it for processing
          const reqAddr = text.slice(2).trim();
          const record: ActionRecord = {
            action: "getBalance",
            field: { address: reqAddr },
            from: packet.from,
            user: reqAddr,
            channel: packet.channel,
            timestamp: Date.now(),
          ...
          // Balance response — check if it's for our address
          const payload = text.slice(2);
          const pipeIdx = payload.indexOf("|");
          const respAddr = pipeIdx !== -1 ? payload.slice(0, pipeIdx) : payload;
          const rawBal = pipeIdx !== -1 ? payload.slice(pipeIdx + 1) : "0";
          // Try to update walletBalance if address matches
          (async () => {
            try {
              const signer = await getEvvmSigner();
              if (signer.address.toLowerCase() === respAddr.toLowerCase()) {
                let display: string;
                try {
                  display = (BigInt(rawBal) / BigInt(10) ** BigInt(18)).toString();
                } catch {
                  display = rawBal;
                }
                setWalletBalance({ address: respAddr, raw: rawBal, display });
              }
            } catch { /* no signer available */ }
          })();
          setMessages((prev) => [
            ...prev,
            {
  ...
  const requestBalance = async () => {
    ...
    try {
      const signer = await getEvvmSigner();
      const msg = `B=${signer.address}`;
      await device.sendText(msg, undefined, true, channel);
      console.log("[Balance] Sent balance request:", msg);
    } catch (error) {
      console.error("[Balance] Error requesting balance:", error);
    }
  };
  ...
  return (
    <Stack p="md" >
      ...
      <Paper withBorder p="md" style={{
        ...
        <Title order={2} mb="sm">Balance Query</Title>
        <Text size="sm" c="dimmed" mb="sm">Request your native token balance via the mesh network</Text>
        <Button onClick={requestBalance} disabled={!isConnected}>Request Balance</Button>
        {walletBalance && (
          <Paper withBorder p="sm" mt="sm" style={{ backgroundColor: "var(--ctp-mantle)", borderLeft: "4px solid var(--ctp-teal)", width: "100%" }}>
            <Text size="sm" style={{ wordBreak: "break-all" }}><strong>Address:</strong> {walletBalance.address}</Text>
            <Text size="sm"><strong>Balance:</strong> {walletBalance.display} <span style={{ color: "var(--ctp-overlay1)" }}>(raw: {walletBalance.raw})</span></Text>
          </Paper>
        )}
      </Paper>
      ...
      */}
      ...
      {/* Action Queue */}
      <Paper withBorder p="md" style={{ backgroundColor: "var(--ctp-surface0)" }}>
        <Title order={2} mb="sm">Action Queue</Title>
        <Group mb="sm">
          <Button
            ...
            onClick={async () => {
              ...
              for (const record of snapshot) {
                ...
                // Handle getBalance separately
                if (record.action === "getBalance") {
                  const balAddr = (record.field as Record<string, any>).address ?? userAddr;
                  try {
                    const res = await fetch(`/api/getBalance?user=${encodeURIComponent(balAddr)}&token=0x0000000000000000000000000000000000000000`);
                    const data = await res.json();
                    const balance = data.balance ?? "0";
                    console.log(`[Queue] getBalance for ${balAddr} → ${balance}`);
                    ...
                    // Send R= reply via Meshtastic
                    if (device && isConnected) {
                      const replyMsg = `R=${balAddr}|${balance}`;
                      try {
                        await device.sendText(replyMsg, undefined, true, channel);
                        console.log("[Queue] Sent balance reply:", replyMsg);
                      } catch (meshErr) {
                        console.error("[Queue] Failed to send balance reply:", meshErr);
                      }
                    }
                  } catch (err) {
                    console.error(`[Queue] getBalance for ${balAddr} failed:`, err);
        ...
        {messages.length === 0 ? (
          ...
          <Stack gap="sm">
            {messages.map((msg, i) => {
              ...
              if (msg.source === "balance-req") {
                const addr = msg.text.slice(2).trim();
                return (
                  <Paper key={i} withBorder p="sm" style={{ backgroundColor: "var(--ctp-mantle)", borderLeft: "4px solid var(--ctp-yellow)" }}>
                    <Text size="sm" fw={600} style={{ color: "var(--ctp-yellow)" }}>Balance Request</Text>
                    <Text size="sm" style={{ wordBreak: "break-all" }}><strong>Address:</strong> {addr}</Text>
                    <Text size="xs" c="dimmed">Channel: {msg.channel} &middot; Node: {msg.from} &middot; {new Date(msg.timestamp).toLocaleString()}</Text>
                  </Paper>
                );
              }
              if (msg.source === "balance-res") {
                const payload = msg.text.slice(2); // strip "R="
                const parts = payload.split("|");
                const addr = parts[0] ?? "";
                const rawBalance = parts[1] ?? "0";
                let display: string;
                try {
                  display = (BigInt(rawBalance) / BigInt(10) ** BigInt(18)).toString();
                } catch {
                  display = rawBalance;
                }
                return (
                  <Paper key={i} withBorder p="sm" style={{ backgroundColor: "var(--ctp-mantle)", borderLeft: "4px solid var(--ctp-teal)" }}>
                    <Text size="sm" fw={600} style={{ color: "var(--ctp-teal)" }}>Balance Response</Text>
                    <Text size="sm" style={{ wordBreak: "break-all" }}><strong>Address:</strong> {addr}</Text>
                    <Text size="sm"><strong>Balance:</strong> {display} <span style={{ color: "var(--ctp-overlay1)" }}>(raw: {rawBalance})</span></Text>
                    <Text size="xs" c="dimmed">Channel: {msg.channel} &middot; Node: {msg.from} &middot; {new Date(msg.timestamp).toLocaleString()}</Text>
                  </Paper>
                );
              }
